...
public:
	...
	Vaccum(Grid &grid, unsigned int StartID, unsigned int EndID)
	{
		iEndID = EndID;
		if (grid.getTileValue(StartID) == '|')
		{
	...
	Vaccum(Grid &grid, unsigned int tileID, bool bIdleOn)
	{
		...
		{
			grid.setTileValue(tileID, '~');
			this->setPosX(grid.getTilePosX(tileID));
			this->setPosY(grid.getTilePosY(tileID));
			this->setPosZ(grid.getTilePosX(tileID));
			this->gridSize = grid.getCol();
		}
	}
	Vaccum(Grid &grid, unsigned int tileID)
	{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void DFS(Grid &grid, int previousstate, int currentState, bool canIMove)
	{
		++stepsTaken;
		...
		if (canIMove)
		{
			...
			{
				nextMove = getNeighbors05(grid, currentState);
				grid.setTileNeigborsFound(currentState, true);
			}

			DFS(grid, currentState, nextMove,canIMove);
		}
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
	void Clean03(Grid &grid, int previousstate, int currentState, bool bIdleOn)
	{
		...
		if (!isFinished)
		{
			...
			if (grid.isTileNeighborsFound(currentState) == false)
			{
				moves = getNeighbors04(grid, currentState);
				grid.setTileNeigborsFound(currentState, true);
			}
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
	vector<int> getNeighbors06(Grid &grid, int currentState, float distance)
	{
		vector<int> moves;
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
	int getNeighbors05(Grid &grid, int currentState)
	{//DFS
		float BESTDISATNCE = 0;
		...
		if (!(grid.getTileID(currentState) % gridSize == 0))
		{
			...
			if (grid.isTileVisited(currentState - 1) == false)
			{
				...
				if (grid.getTileValue(currentState - 1) != '|')
				{
					float DISTANCE = sqrt
					(
		...
		if (grid.getTileID(currentState) < (gridSize * gridSize) - gridSize)
		{
			...
			{
				//CHECK if LEFT NODE IS OBSTICLE
				if (grid.getTileValue((currentState + gridSize) != '|'))
				{
					float DISTANCE = sqrt
					(
